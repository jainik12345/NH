...
const OursHotelsSubNavbarGallery = () => {
  const { hotelSlug } = useParams();
  const hotelInfo = SubNavbarHotelData[hotelSlug];

  if (!hotelInfo || !hotelInfo.navbarGallery) {
    return (
      <div className="text-center py-10">Gallery information not found.</div>
    );
  }

  const galleryData = hotelInfo.navbarGallery.sections;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {filteredImages.map((img, idx) => (
              <img
                ...
                className="w-full h-[250px] object-cover hover:scale-101 cursor-pointer transition-transform duration-300"
              />
            ))}
          </div>
        </div>
      ...
      <AnimatePresence>
        {popupOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-transparent bg-opacity-80 flex items-center justify-center z-50 backdrop-blur-[4px]"
          >
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
...
export default OursHotelsSubNavbarGallery;
